...
description: This skill should be used when users need to extract subtitles from videos using OCR (Optical Character Recognition) technology. It identifies and extracts hard-coded subtitles from video frames, preserving timing information and generating SRT format files.
---
...
Extract subtitles from videos using advanced OCR technology. Automatically detects subtitle text, timing, and generates properly formatted SRT files with 98%+ accuracy.
...
The video-extraction skill handles the following workflow:
...
- `task_type`: "video-extraction" - Type of processing task
